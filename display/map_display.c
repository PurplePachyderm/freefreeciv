...
void dispBackground(SDL_Window * window, SDL_Renderer * renderer, SDL_Surface * sprites, SDL_Texture * texture, view camera){
	...
	while(currentPos.y < SCREEN_HEIGHT){
		...
		while(currentPos.x < SCREEN_WIDTH){
			...
			if( (currentPos.x < camera.offset.x+width || currentPos.x > camera.offset.x+(MAP_SIZE-2)*width) ||
			((currentPos.y < camera.offset.y+width || currentPos.y > camera.offset.y+(MAP_SIZE-2)*width)) ){
...
int cameraEvents(SDL_Event * event, view * camera){	//Changes camera's zoom & offset
	...
	while(SDL_PollEvent(event)){
		...
		if(event->type == SDL_MOUSEMOTION && camera->leftClick){
			...
			if(camera->offset.x < -TILE_SIZE*camera->zoom*MAP_SIZE)
				camera->offset.x = -TILE_SIZE*camera->zoom*MAP_SIZE;
			if(camera->offset.x > SCREEN_WIDTH)
				camera->offset.x = SCREEN_WIDTH;
			if(camera->offset.y < -TILE_SIZE*camera->zoom*MAP_SIZE)
				camera->offset.y = -TILE_SIZE*camera->zoom*MAP_SIZE;
